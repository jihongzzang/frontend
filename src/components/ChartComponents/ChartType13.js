...
const ChartType13 = () => {
  const conversion = useRecoilValue(chartData6);
  const conversion2 = useRecoilValue(chartData11);
  const week = useRecoilValue(selectedWeeks);

  const convertArr = Object.values(conversion);
  const convertArr2 = Object.values(conversion2);

  const parsingData = {
    [week]: convertArr.slice(1, convertWeeks(week, conversion)),
  };

  const parsingData2 = {
    [week]: convertArr2.slice(1, convertWeeks(week, conversion2)),
  };
  ...
  const renderData = parsingData[week].map(
    ele => (ele.hashtagClick / ele.actions) * 100
  );
    ...
    <StyledDataBox size="large" color="borderColor" outline>
      <Header>
        <LegendDataBox size="medium" color="black">
          <h2>
            광고 게시물에서 공식계정으로 전환률 <span>(주차별)</span>
          </h2>
        </LegendDataBox>
      </Header>
      <Content>
