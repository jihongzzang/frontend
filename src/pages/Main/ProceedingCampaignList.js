...
const ProceedingCampaignList = () => {
  ...
  const proceedingList = proceeding.sort(function (a, b) {
    ...
    <StyledDataBox size="large" color="borderColor" outline>
      <Header>
        <LegendDataBox size="medium" color="black">
          <h2>현재 진행 중인 캠페인 (총 {proceedingList.length} 건)</h2>
        </LegendDataBox>
        ...
        <TableContent>
          {proceedingList.map((campaign, idx) => {
            return (
...
export default ProceedingCampaignList;
