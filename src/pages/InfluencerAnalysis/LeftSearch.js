...
import {
  leftfilteredInfluencers,
  leftSelectedCampaign,
  leftSelectedInfluencer,
...
const LeftSearch = () => {
  const [campaign, setCampaign] = useRecoilState(leftSelectedCampaign);
  const [influencer, setInfluencer] = useRecoilState(leftSelectedInfluencer);
  const campaignsList = useRecoilValue(campaignListSelector2);
  const influencersList = useRecoilValue(leftfilteredInfluencers);
